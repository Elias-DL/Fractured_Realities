...
public class LagaeMonster : MonoBehaviour // reset de component voor changes 
{
    public enum Zone
    {
    ...
    private void Update()
    {
        ...
        playerGezien = Player.GetComponent<PlayerMovement>().gezien; // gezien nog opnieuw programmeren in player movement

        if (playerGezien == true)
        {
            animator.SetBool("Roam", true);
            Debug.Log("monster gezien");
            action = "Roam";
            RoamAround();
    ...
    private void RoamAround()
    {
        ...
        if (!navAgent.pathPending && navAgent.remainingDistance <= navAgent.stoppingDistance || (playerGezien && !destinationSet))
        {
            ...
            if (NavMesh.SamplePosition(randomDirection, out hit, roamRange, NavMesh.AllAreas))
            {
                ...
                // If the player is seen, mark that the destination has been set
                if (playerGezien)
                {
                    destinationSet = true;
                }
        ...
        // Reset destinationSet when the monster reaches the target
